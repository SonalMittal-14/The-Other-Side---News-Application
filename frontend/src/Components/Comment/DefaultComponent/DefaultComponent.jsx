...
console.log(user);
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const uri = `${window.API_URL}/getcomments/${articleId}`;
  ...
  const handleLikeComment = async (commentId) => {
    try {
      ...
      const response = await axios.post(`${window.API_URL}/likecomment/${commentId}`, {}, { 
        headers : {
          Authorization : `${token}`
        }
       });
  ...
  const handleUnlikeComment = async (commentId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(`${window.API_URL}/unlikecomment/${commentId}`, {}, { 
        headers : {
  ...
  const handleCommentSubmit = async (newComment) => {
    try {
      const response = await axios.post(`${window.API_URL}/addcomment`, {
  ...
  const handleReplySubmit = async (commentId, replyText) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(`${window.API_URL}/replycomment/${commentId}`, {
        text: replyText,
      }, { 
        headers : {
          'Authorization' : `${token}`
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const response = await axios.delete(
        `${window.API_URL}/deletecomment/${commentId}`,
        { withCredentials: true }
      );
  ...
  const handleEditComment = async (commentId, newText) => {
    try {
      const response = await axios.put(
       `${window.API_URL}/updatecomment/${commentId}`,
        { text: newText },
        { withCredentials: true }
      );
