...
const ArticleUpdateForm = () => {
    ...
    useEffect(() => {
        const fetchUpdateArticleDetails = async () => {
            try {
                const uri = `${window.API_URL}/getArticle/${id}`;
    ...
    const submitUpdateArticleForm = async (event)=> {
        ...
        try{
            const uri = `${window.API_URL}/updateArticle/${id}`;
            const token  = localStorage.getItem(token);
  ...
  return (
    <form onSubmit={submitUpdateArticleForm} class="body-font min-h-screen relative bg-gray-900 text-gray-400">
        ...
        <div class="mx-auto md:w-2/3 lg:w-1/2 mt-12 ">
          <div class=" flex flex-wrap gap-y-10">
            <div class=" flex items-center justify-center flex-col border-2 w-full border-slate-700 border-dotted px-5 py-2 rounded-xl">
              ...
              <img src={file ? URL.createObjectURL(file) : fileUrl} alt="" />
            </div>
            ...
            <Select
              ...
              options={options}
              styles={customStyles}
              value={options.find(option => option.value === updatenewsPayload.category)}
